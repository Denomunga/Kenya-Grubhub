...
export const procurementApi = {
  ...
  approveRequest: (id: string, notes?: string) =>
    apiFetch(`${BASE}/purchase-requests/${id}/approve`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ notes }) }).then(res => res.json()),
  rejectRequest: (id: string, reason: string) =>
    apiFetch(`${BASE}/purchase-requests/${id}/reject`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ reason }) }).then(res => res.json()),
  ...
  confirmPO: (id: string) =>
    apiFetch(`${BASE}/purchase-orders/${id}/confirm`, { method: 'POST', headers: { 'Content-Type': 'application/json' } }).then(res => res.json()),
  cancelPO: (id: string, reason: string) =>
    apiFetch(`${BASE}/purchase-orders/${id}/cancel`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ reason }) }).then(res => res.json()),
